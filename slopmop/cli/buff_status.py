"""``sm buff status`` / ``sm buff watch`` — PR CI status reporting.

Split out of ``buff.py`` to keep that module under the code-sprawl ceiling.
This module owns the status-polling command plus the helpers that only the
status path uses (late-feedback detection and the unresolved-feedback blocker
renderer).
"""
