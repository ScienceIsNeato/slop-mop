...
from slopmop.cli.buff_common import (
    warn_if_pr_worktree_mismatch as _warn_if_pr_worktree_mismatch,
)
...
from slopmop.cli.buff_rounds import (
    note_for_completed_round as _note_for_completed_round,
)
from slopmop.cli.buff_scan import print_scan_unavailable, run_inspect_scan
from slopmop.cli.buff_status import cmd_buff_status as _cmd_buff_status
...
def _cmd_buff_resolve(
    ...
    round_note = ""
    if resolve_thread:
        # A resolve that clears the last open thread completes a feedback
        # round — stamp the boat with another jolly roger via the PR label.
        round_note = _note_for_completed_round(
            project_root, owner, repo, resolved_pr_number
        )

    action = "commented and resolved" if resolve_thread else "commented"
    print(
        f"Buff resolve complete: {action} {thread_id} "
        f"on PR #{resolved_pr_number}.{round_note}"
    )
    return 0
