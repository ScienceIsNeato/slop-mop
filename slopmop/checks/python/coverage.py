...
class PythonNewCodeCoverageCheck(BaseCheck, PythonCheckMixin):
    """Coverage on new/changed code (CI alias for diff-coverage).

    Wraps diff-cover identically to python:diff-coverage. This gate
    exists under the new-code-coverage name for CI workflow compat.

    Profiles: pr

    Configuration:
      Same as python:diff-coverage. Threshold: 80% on changed lines.
      Compare branch resolved from env vars or origin/main.
