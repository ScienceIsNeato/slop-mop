...
TAUTOLOGY_PATTERNS: List[Tuple[Pattern, str]] = [
    ...
    (re.compile(r"expect\s*\(\s*true\s*\)\s*\.toBe\s*\(\s*true\s*\)"), "expect(true).toBe(true)"),
    (re.compile(r"expect\s*\(\s*false\s*\)\s*\.toBe\s*\(\s*false\s*\)"), "expect(false).toBe(false)"),
    # expect(true).toEqual(true)
    (re.compile(r"expect\s*\(\s*true\s*\)\s*\.toEqual\s*\(\s*true\s*\)"), "expect(true).toEqual(true)"),
    (re.compile(r"expect\s*\(\s*false\s*\)\s*\.toEqual\s*\(\s*false\s*\)"), "expect(false).toEqual(false)"),
    # expect(1).toBe(1) or any single digit
    (re.compile(r"expect\s*\(\s*(\d+)\s*\)\s*\.toBe\s*\(\s*\1\s*\)"), "expect(N).toBe(N)"),
    (re.compile(r"expect\s*\(\s*(\d+)\s*\)\s*\.toEqual\s*\(\s*\1\s*\)"), "expect(N).toEqual(N)"),
    # expect('string').toBe('string')
    (re.compile(r"expect\s*\(\s*['\"]([^'\"]+)['\"]\s*\)\s*\.toBe\s*\(\s*['\"](\1)['\"]\s*\)"), "expect('x').toBe('x')"),
    # expect().toBeTruthy() on literal true
    (re.compile(r"expect\s*\(\s*true\s*\)\s*\.toBeTruthy\s*\(\s*\)"), "expect(true).toBeTruthy()"),
    # expect().toBeFalsy() on literal false
    (re.compile(r"expect\s*\(\s*false\s*\)\s*\.toBeFalsy\s*\(\s*\)"), "expect(false).toBeFalsy()"),
]
...
ALTERNATIVE_ASSERTIONS: List[Pattern] = [
    ...
    re.compile(r"\bfindBy"),  # findByText, findByTestId, etc. - async queries that throw
    ...
    for pattern in [TEST_DEFINITION_PATTERN, TEST_FUNCTION_PATTERN]:
        for match in pattern.finditer(content):
            ...
            if _is_empty_body(body):
                findings.append(BogusTestFinding(
                    file=rel_path,
                    test_name=test_name,
                    line=line_num,
...
class JavaScriptBogusTestsCheck(BaseCheck, JavaScriptCheckMixin):
    ...
    def run(self, project_root: str) -> CheckResult:
        ...
        if len(findings) <= max_allowed:
            output_lines.insert(0, f"⚠️ {len(findings)} bogus tests found (max allowed: {max_allowed})")
            return self._create_result(
                status=CheckStatus.PASSED,
                duration=duration,
                output="\n".join(output_lines),
