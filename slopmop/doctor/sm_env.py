...
Checks about *slop-mop's own* install — the Python it runs in and the
shell block it presses — not the user's project.
...
``sm_env.gang`` — is the pressed shell block still the one this version
generates?  ``~/.slopmop/aliases.sh`` is written into ``$HOME`` once and
was then never revisited, so wrapper fixes reached the source and no
machine: an install pressed at 2.0.0 was still running 2.0.0 wrappers
fifteen minor releases later, with enforcement silently disabled the
whole time.  Reports only — regenerating shell intercepts machine-wide
is not something ``--fix`` should do behind your back.
"""
...
class GangFreshnessCheck(DoctorCheck):
    name = "sm_env.gang"
    description = "Pressed shell intercepts match the running slopmop version"

    # Regenerating machine-wide shell intercepts is not a repair to make
    # without being asked: a newer mapping can intercept commands the
    # installed block never touched. Report, hand over the command, let the
    # human run it.
    can_fix = False
    ...
    def run(self, ctx: DoctorContext) -> DoctorResult:
        ...
        from slopmop.cli.gang import (
            GANG_CONFIRM_PHRASE,
            gang_is_pressed,
            installed_gang_version,
        )

        if not gang_is_pressed():
            return self._skip("no gang block pressed on this machine")
        ...
        repress = f'sm gang press --confirm "{GANG_CONFIRM_PHRASE}"'

        if installed is None:
            return self._warn(
                "pressed aliases.sh carries no version stamp",
                detail=(
                    "~/.slopmop/aliases.sh has no 'Generated by sm gang vX' "
                    "header, so it was hand-edited or written by a version "
                    "predating the stamp. Its wrappers cannot be assumed to "
                    "match this slopmop."
                ),
                fix_hint=repress,
                data=data,
        ...
        if installed == __version__:
            return self._ok(f"gang block current (v{installed})", data=data)

        return self._warn(
            f"gang block was pressed by v{installed}, running v{__version__}",
            detail=(
                f"~/.slopmop/aliases.sh was generated by slopmop {installed}.\n"
                "Wrapper generation has changed since; the intercepts in your\n"
                "shells are whatever that version emitted, not what this one\n"
                "would. Fixes to the wrappers do not reach a machine until it\n"
                "is pressed again."
            ),
            fix_hint=repress,
            data=data,
