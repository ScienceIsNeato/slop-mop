...
class TestJavaScriptCoveragePctCoercion:
    """Regression tests for the ``"pct": "Unknown"`` crash.

    Found during Phase-2 beta testing on ai-tic-tac-toe-game: Jest's
    ``coverage-summary.json`` emits ``"pct": "Unknown"`` (a string) for
    files with zero executable statements.  The gate would then raise
    ``TypeError: '>=' not supported between 'str' and 'int'`` at the
    threshold comparison and ERROR the whole run instead of reporting
    coverage.
    """
    ...
    def test_evaluate_coverage_mixed_unknown_and_numeric(self):
        """Realistic case: one type-only barrel file, rest has real coverage."""
