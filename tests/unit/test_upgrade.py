...
class TestRepressGangOnUpgrade:
    """Upgrading is the moment the generated shell block is known to be behind.

    ``~/.slopmop/aliases.sh`` was written into ``$HOME`` once and never
    revisited, so wrapper fixes landed in the source and reached no machine —
    one install ran 2.0.0's wrappers fifteen minor releases later with shell
    enforcement silently off the whole time.
    """

    @staticmethod
    def _pressed(tmp_path: Path, monkeypatch) -> None:
        import slopmop.cli.gang as gang_mod

        dest = tmp_path / "aliases.sh"
        dest.write_text("# Generated by sm gang v2.0.0 — DO NOT EDIT\n")
        monkeypatch.setattr(gang_mod, "_ALIASES_DEST", dest)

    @staticmethod
    def _unpressed(tmp_path: Path, monkeypatch) -> None:
        import slopmop.cli.gang as gang_mod

        monkeypatch.setattr(gang_mod, "_ALIASES_DEST", tmp_path / "absent.sh")

    def test_untouched_when_nothing_was_ever_pressed(
        self, tmp_path: Path, monkeypatch
    ) -> None:
        """Consent is not implied by upgrading.

        Pressing a block on a machine that never opted in would alias commands
        system-wide as a side effect of ``sm upgrade``.
        """
        from slopmop.cli import upgrade as upgrade_mod

        self._unpressed(tmp_path, monkeypatch)
        with patch.object(upgrade_mod, "bounded_run") as mock_run:
            assert upgrade_mod._repress_gang("2.15.0") is None
        assert not mock_run.called

    def test_repressed_when_a_block_exists(self, tmp_path: Path, monkeypatch) -> None:
        from slopmop.cli import upgrade as upgrade_mod

        self._pressed(tmp_path, monkeypatch)
        with patch.object(upgrade_mod, "bounded_run") as mock_run:
            mock_run.return_value = subprocess.CompletedProcess(
                args=["python"], returncode=0, stdout="", stderr=""
            )
            note = upgrade_mod._repress_gang("2.15.0")

        assert note is not None and "2.15.0" in note
        assert mock_run.called

    def test_repress_runs_out_of_process(self, tmp_path: Path, monkeypatch) -> None:
        """In-process generation would stamp OLD wrappers as the NEW version.

        After the install this interpreter still holds the pre-upgrade
        slopmop, so calling ``_gang_press`` directly would write the previous
        version's wrappers under the new version's header — stale content
        labelled fresh, which defeats the staleness check outright.
        """
        from slopmop.cli import upgrade as upgrade_mod
        ...
        with patch.object(upgrade_mod, "bounded_run") as mock_run:
            ...
            upgrade_mod._repress_gang("2.15.0")

        argv = mock_run.call_args[0][0]
        assert argv[0] == sys.executable
        assert argv[1:3] == ["-m", upgrade_mod.PACKAGE_NAME]
        assert argv[3:5] == ["gang", "press"]
        # Without the phrase the press is a no-op that prints a warning.
        assert "--confirm" in argv

    def test_failed_repress_warns_but_does_not_fail_the_upgrade(
        self, tmp_path: Path, monkeypatch
    ) -> None:
        """The package installed cleanly; a stale shell block is not a rollback."""
        from slopmop.cli import upgrade as upgrade_mod

        self._pressed(tmp_path, monkeypatch)
        with patch.object(upgrade_mod, "bounded_run") as mock_run:
            mock_run.return_value = subprocess.CompletedProcess(
                args=["python"], returncode=1, stdout="", stderr="permission denied\n"
            )
            note = upgrade_mod._repress_gang("2.15.0")

        assert note is not None
        assert "permission denied" in note
        assert "sm gang press" in note

    def test_repress_crash_is_reported_not_raised(
        self, tmp_path: Path, monkeypatch
    ) -> None:
        from slopmop.cli import upgrade as upgrade_mod
        ...
        assert note is not None
        assert "no such interpreter" in note
        assert "sm gang press" in note

    def test_timeout_is_reported_not_raised(self, tmp_path: Path, monkeypatch) -> None:
        """bounded_run raises TimeoutExpired rather than swallowing it."""
        from slopmop.cli import upgrade as upgrade_mod

        self._pressed(tmp_path, monkeypatch)
        with patch.object(upgrade_mod, "bounded_run") as mock_run:
            mock_run.side_effect = subprocess.TimeoutExpired(cmd="sm", timeout=1)
            note = upgrade_mod._repress_gang("2.15.0")

        assert note is not None and "sm gang press" in note

    def test_note_reaches_the_summary(self, tmp_path: Path, capsys) -> None:
        """A refresh the user never sees is indistinguishable from none."""
        from slopmop.cli import upgrade as upgrade_mod
        ...
        upgrade_mod._print_upgrade_summary(
            ...
            project_root=tmp_path,
            gang_note="🔗 Gang block re-pressed for 2.15.0",
        )
        assert "Gang block re-pressed for 2.15.0" in capsys.readouterr().out
